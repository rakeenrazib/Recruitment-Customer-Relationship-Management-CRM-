<x-app-layout>
    @php
        $typeColors = [
            ...
            'evaluation-published' => 'bg-cyan-100 text-cyan-700',
            'evaluation-updated' => 'bg-teal-100 text-teal-700',
        ];
    @endphp
    ...
    <div class="py-8">
        <div class="mx-auto max-w-4xl space-y-4 px-4 sm:px-6 lg:px-8">
            ...
            @endif
            @if(session('error'))
                <div class="panel border-red-200 bg-red-50/90 px-5 py-4 text-sm font-medium text-red-800">{{ session('error') }}</div>
            @endif
                    ...
                    <div class="panel px-6 py-5 {{ $notif->is_read ? '' : 'border-sky-200 bg-sky-50/60' }}">
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <span class="pill {{ $typeColors[$notif->type] ?? 'bg-slate-100 text-slate-600' }}">{{ str_replace('_', ' ', $notif->type) }}</span>
                                    @if($targetUrl)
                                        <a href="{{ $targetUrl }}" class="mt-3 block text-sm font-semibold leading-6 text-slate-900 transition hover:text-teal-700">
                                            {{ $notif->message }}
                                        </a>
                                    @else
                            ...
                            <div class="flex items-center gap-3">
                                @if($targetUrl)
                                    <a href="{{ $targetUrl }}" class="text-sm font-bold text-teal-700 transition hover:text-teal-900">View</a>
                                @endif
                                @if(!$notif->is_read)
